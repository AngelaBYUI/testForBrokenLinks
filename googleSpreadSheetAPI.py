...
try: #get the file and the specific sheet
    spreadsheet = gc.open_by_key('14E2jkyH9Oo26kDikeIip9FWQzEarPTqSkutRxK9Kk-g') #the key is url for the sheet but only the strings between /d/ and /edit
    ...
    try: #get the dta from specific column but restrict in the len(rows) bc we don't want to mark a thousand empty lines.
        ...
        col_data = worksheet.col_values(6)  # column 'A' is 1, 'B' is 2, and so on.
        ...
        cell_to_mark_broken=[]
        i=0
        link_check_count = 0 #for the sleep(); gonna make it sleep every ten links
        # for row, cell in enumerate(col_data, start=1): #This line for all data
        for row in range(1172, num_rows + 1): #This line and next line are for starting from specific data
            cell = col_data[row - 1] #for staring from specific data
